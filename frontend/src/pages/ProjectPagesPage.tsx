...
export function ProjectPagesPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="text-sm">
            <Link className="text-link hover:underline" to="/projects">
          ...
          <p className="mt-1 text-sm text-fg-muted">{t('pages.subtitle')}</p>
        </div>
      </div>
      ...
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="flex-1 space-y-1">
            ...
            />
            {nameError && (
              <div className="text-xs text-danger">{t(nameError)}</div>
            )}
          </label>
          <button
            type="submit"
      ...
      {loading ? (
        <div className="text-sm text-fg-muted">{t('editor.loading')}</div>
      ) : error ? (
        <div className="space-y-2 rounded-lg border border-danger-border bg-danger-bg p-4 text-sm text-danger-fg">
          <div>{t('pages.loadError')}</div>
          <button
            type="button"
            className="rounded-md border border-danger-border bg-surface px-3 py-1.5 text-sm text-danger-fg"
          ...
          {pages.map((p) => (
            ...
            >
              <div>
                ...
                <div className="mt-1 text-xs text-fg-subtle">{p.slug}</div>
              </div>
              <Link
